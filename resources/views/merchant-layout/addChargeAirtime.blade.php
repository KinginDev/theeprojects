                                ...
                                <div class="card moni-card br-2">
                                    ...
                                    <form id="chargeAirtimeForm">
                                        ...
                                        <div class="m-3">
                                            ...
                                            <input type="text" class="form-control" name="smart_earners" id="smart_earners"
                                                required value="{{ $airtimes->smart_earners_percent }}">
                                        </div>
                                        <div class="m-3">
                                            <label for="top_users">TOP USERS PERCENTAGE:</label>
                                            <input type="text" class="form-control" name="top_users" id="top_users" required
                                                value="{{ $airtimes->topuser_earners_percent }}">
                                        </div>
                                        <div class="m-3">
                                            <label for="api_percent">API PERCENTAGE:</label>
                                            <input type="text" class="form-control" name="api_percent" id="api_percent"
                                                required value="{{ $airtimes->api_earners_percent }}">
                                        </div>
                                        <div class="m-3">
    ...
    <script>
        $(document).ready(function () {
            $('#chargeAirtimeForm').on('submit', function (event) {
                ...
                $.ajax({
                    url: '{{ route('merchant.addChargeAirtime.user', ['id' => $airtimes->id]) }}',
                    method: 'POST',
                    data: formData,
